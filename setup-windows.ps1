    ...
    if (Test-Path $Target) {
        ...
        if ($item.Attributes -band [System.IO.FileAttributes]::ReparsePoint) {
            ...
            if ($linkTarget -eq $Source) {
                ...
                Write-Host "A symlink already exists for $SubDirectory but points to: $linkTarget" -ForegroundColor Yellow
                $confirm = Read-Host "Remove existing symlink and create new one? (y/n)"
                if ($confirm -ne 'y') {
